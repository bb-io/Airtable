...
public class AttachmentFieldDataSourceHandler : AirtableInvocable, IAsyncDataSourceHandler
{
    private readonly FieldAndRecordIdentifier _field;

    public AttachmentFieldDataSourceHandler(InvocationContext invocationContext, [ActionParameter] FieldAndRecordIdentifier field) : base(
        invocationContext)
    {
        _field = field;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return table.Fields
            .Where(x => x.Type == "multipleAttachments" && (context.SearchString is null ||
