...
public class DateFieldDataSourceHandler : AirtableInvocable, IAsyncDataSourceHandler
{
    private readonly FieldAndRecordIdentifier _field;

    public DateFieldDataSourceHandler(InvocationContext invocationContext, [ActionParameter] FieldAndRecordIdentifier field) : base(
        invocationContext)
    {
        _field = field;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return table.Fields
            .Where(x => x.Type == "date" && (context.SearchString is null ||
