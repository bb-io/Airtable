...
public class UniqueFieldDataSourceHandler : AirtableInvocable, IAsyncDataSourceHandler
{
    private readonly UniqueFieldIdentifier _field;

    public UniqueFieldDataSourceHandler(InvocationContext invocationContext, [ActionParameter] UniqueFieldIdentifier field)
    ...
    public async Task<Dictionary<string, string>> GetDataAsync(DataSourceContext context, CancellationToken cancellationToken)
    {
        ...
        if (table == null)
            throw new Exception($"Could not find table with ID {_field.TableId}");
