...
public class BoolFieldDataSourceHandler : AirtableInvocable, IAsyncDataSourceHandler
{
    private readonly FieldAndRecordIdentifier _field;

    public BoolFieldDataSourceHandler(InvocationContext invocationContext, [ActionParameter] FieldAndRecordIdentifier field) : base(
        invocationContext)
    {
        _field = field;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return table.Fields
            .Where(x => x.Type == "checkbox" && (context.SearchString is null ||
